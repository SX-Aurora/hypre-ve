...
hypre_CGNRFunctions *hypre_CGNRFunctionsCreate(
  ...
  cgnr_functions = (hypre_CGNRFunctions *)hypre_CTAlloc(hypre_CGNRFunctions, 1,
                                                        HYPRE_MEMORY_HOST);
...
void *hypre_CGNRCreate(hypre_CGNRFunctions *cgnr_functions) {
  ...
  if (avals_ptr == NULL) {
   ...
   //  fprintf(stderr, "CGNR array of pointers has been allocated\n");
  }
#endif
...
HYPRE_Int hypre_CGNRSetup(void *cgnr_vdata, void *A, void *b, void *x) {
  ...
  if ((cgnr_data->logging) > 0) {
    (cgnr_data->norms) =
        hypre_CTAlloc(HYPRE_Real, max_iter + 1, HYPRE_MEMORY_HOST);
...
HYPRE_Int hypre_CGNRSolve(void *cgnr_vdata, void *A, void *b, void *x) {
  ...
  HYPRE_Int (*precond)(void *, void *, void *, void *) =
      (cgnr_functions->precond);
  HYPRE_Int (*precondT)(void *, void *, void *, void *) =
      (cgnr_functions->precondT);
  void *precond_data = (cgnr_data->precond_data);
  ...
  if (ieee_check != ieee_check) {
    ...
    if (logging > 0) {
      ...
      hypre_printf(
          "ERROR -- hypre_CGNRSolve: INFs and/or NaNs detected in input.\n");
  ...
  if (logging > 0) {
    ...
    /* Since it is does not diminish performance, attempt to return an error
       flag and notify users when they supply bad input. */
    if (norms[0] != 0.)
      ieee_check = norms[0] / norms[0]; /* INF -> NaN conversion */
    if (ieee_check != ieee_check) {
      /* ...INFs or NaNs in input can make ieee_check a NaN.  This test
         for ieee_check self-equality works on all IEEE-compliant compilers/
         machines, c.f. page 8 of "Lecture Notes on the Status of IEEE 754"
    ...
    if (logging > 0) {
      ...
      if (logging > 1 && my_id == 0) {
        hypre_printf("% 5d    %e    %f   %e\n", i, norms[i],
                     norms[i] / norms[i - 1], norms[i] / bi_prod);
      }
    }
...
HYPRE_Int hypre_CGNRGetPrecond(void *cgnr_vdata,
...
HYPRE_Int hypre_CGNRGetNumIterations(void *cgnr_vdata,
