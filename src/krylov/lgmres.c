...
hypre_LGMRESFunctions *hypre_LGMRESFunctionsCreate(
  ...
  lgmres_functions = (hypre_LGMRESFunctions *)CAlloc(
      1, sizeof(hypre_LGMRESFunctions), HYPRE_MEMORY_HOST);
  ...
  lgmres_functions->CreateVectorArray =
      CreateVectorArray; /* not in PCGFunctionsCreate */
...
void *hypre_LGMRESCreate(hypre_LGMRESFunctions *lgmres_functions) {
  ...
  lgmres_data =
      hypre_CTAllocF(hypre_LGMRESData, 1, lgmres_functions, HYPRE_MEMORY_HOST);
  ...
  if (avals_ptr == NULL) {
   ...
   //  fprintf(stderr, "LGMRES array of pointers has been allocated\n");
  }
#endif
...
HYPRE_Int hypre_LGMRESSetup(void *lgmres_vdata, void *A, void *b, void *x) {
  ...
  HYPRE_Int (*precond_setup)(void *, void *, void *, void *) =
      (lgmres_functions->precond_setup);
  ...
  if ((lgmres_data->p) == NULL)
    (lgmres_data->p) =
        (void **)(*(lgmres_functions->CreateVectorArray))(k_dim + 1, x);
  if ((lgmres_data->r) == NULL)
  ...
  /* lgmres mod */
  if ((lgmres_data->aug_vecs) == NULL)
    (lgmres_data->aug_vecs) = (void **)(*(lgmres_functions->CreateVectorArray))(
        aug_dim + 1, x); /* one extra */
  if ((lgmres_data->a_aug_vecs) == NULL)
    (lgmres_data->a_aug_vecs) =
        (void **)(*(lgmres_functions->CreateVectorArray))(aug_dim, x);
  if ((lgmres_data->aug_order) == NULL)
    (lgmres_data->aug_order) =
        hypre_CTAllocF(HYPRE_Int, aug_dim, lgmres_functions, HYPRE_MEMORY_HOST);
  /*---*/
  ...
  if ((lgmres_data->logging) > 0 || (lgmres_data->print_level) > 0) {
    if ((lgmres_data->norms) == NULL)
      (lgmres_data->norms) = hypre_CTAllocF(
          HYPRE_Real, max_iter + 1, lgmres_functions, HYPRE_MEMORY_HOST);
  }
...
HYPRE_Int hypre_LGMRESSolve(void *lgmres_vdata, void *A, void *b, void *x) {
  ...
  if (ieee_check != ieee_check) {
    ...
    if (logging > 0 || print_level > 0) {
      ...
      hypre_printf(
          "ERROR -- hypre_LGMRESSolve: INFs and/or NaNs detected in input.\n");
  ...
  /* convergence criteria: |r_i| <= max( a_tol, r_tol * den_norm)
      den_norm = |r_0| or |b|
      note: default for a_tol is 0.0, so relative residual criteria is used
     unless user specifies a_tol, or sets r_tol = 0.0, which means absolute
            tol only is checked  */
  ...
  while (iter < max_iter) {
    ...
    while (i < it_total && iter < max_iter) {
      ...
      if (i <= it_arnoldi) { /* Arnoldi */
        ...
        order =
            i - it_arnoldi -
            1; /* which aug step (note i starts at 1) - aug order number at 0*/
        for (ii = 0; ii < aug_dim; ii++) {
          if (aug_order[ii] == order) {
            spot = ii;
            break; /* must have this because there will be duplicates before
                      aug_ct = aug_dim */
          }
        }
    ...
    if (i)
      (*(lgmres_functions->Axpy))(rs[i] - 1.0, p[i], p[i]);
    for (j = i - 1; j > 0; j--)
    ...
    if (aug_dim > 0) {
      ...
      /*need to normalize */
      tmp_norm = sqrt(
          (*(lgmres_functions->InnerProd))(aug_vecs[spot], aug_vecs[spot]));
      ...
      (*(lgmres_functions->ScaleVector))(-tmp_norm,
                                         a_aug_vecs[spot]); /* r0-rm /norm */
    }
  ...
  if (iter >= max_iter && r_norm > epsilon && epsilon > 0)
...
HYPRE_Int hypre_LGMRESSetConvergenceFactorTol(void *lgmres_vdata,
                                              HYPRE_Real cf_tol) {
  hypre_LGMRESData *lgmres_data = (hypre_LGMRESData *)lgmres_vdata;
...
HYPRE_Int hypre_LGMRESGetConvergenceFactorTol(void *lgmres_vdata,
                                              HYPRE_Real *cf_tol) {
  hypre_LGMRESData *lgmres_data = (hypre_LGMRESData *)lgmres_vdata;
...
HYPRE_Int hypre_LGMRESSetPrecond(
    void *lgmres_vdata, HYPRE_Int (*precond)(void *, void *, void *, void *),
    HYPRE_Int (*precond_setup)(void *, void *, void *, void *),
    void *precond_data) {
  hypre_LGMRESData *lgmres_data = (hypre_LGMRESData *)lgmres_vdata;
  hypre_LGMRESFunctions *lgmres_functions = lgmres_data->functions;
...
HYPRE_Int hypre_LGMRESGetPrecond(void *lgmres_vdata,
...
HYPRE_Int hypre_LGMRESGetNumIterations(void *lgmres_vdata,
