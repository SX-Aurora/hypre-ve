...
hypre_GMRESFunctions *hypre_GMRESFunctionsCreate(
  ...
  gmres_functions = (hypre_GMRESFunctions *)CAlloc(
      1, sizeof(hypre_GMRESFunctions), HYPRE_MEMORY_HOST);
  ...
  gmres_functions->CreateVectorArray =
...
void *hypre_GMRESCreate(hypre_GMRESFunctions *gmres_functions) {
  ...
  gmres_data =
      hypre_CTAllocF(hypre_GMRESData, 1, gmres_functions, HYPRE_MEMORY_HOST);
  ...
  if (avals_ptr == NULL) {
   ...
   //  fprintf(stderr, "GMRES array of pointers has been allocated\n");
  }
#endif
...
HYPRE_Int hypre_GMRESSetup(void *gmres_vdata, void *A, void *b, void *x) {
  ...
  if ((gmres_data->p) == NULL)
    (gmres_data->p) =
        (void **)(*(gmres_functions->CreateVectorArray))(k_dim + 1, x);
  if ((gmres_data->r) == NULL)
...
HYPRE_Int hypre_GMRESSolve(void *gmres_vdata, void *A, void *b, void *x) {
  ...
  /* not used yet   char           *log_file_name  = (gmres_data ->
   * log_file_name);*/
  /*   FILE           *fp; */
  ...
  rs =
      hypre_CTAllocF(HYPRE_Real, k_dim + 1, gmres_functions, HYPRE_MEMORY_HOST);
  ...
  for (i = 0; i < k_dim + 1; i++) {
    hh[i] =
        hypre_CTAllocF(HYPRE_Real, k_dim, gmres_functions, HYPRE_MEMORY_HOST);
  }
  ...
  while (iter < max_iter) {
    ...
    if (r_norm <= epsilon && iter >= min_iter) {
      ...
      if (r_norm <= epsilon) {
        if (rel_change &&
            !rel_change_passed) /* calculate the relative change */
        {
...
HYPRE_Int hypre_GMRESSetConvergenceFactorTol(void *gmres_vdata,
                                             HYPRE_Real cf_tol) {
  hypre_GMRESData *gmres_data = (hypre_GMRESData *)gmres_vdata;
...
HYPRE_Int hypre_GMRESGetConvergenceFactorTol(void *gmres_vdata,
                                             HYPRE_Real *cf_tol) {
  hypre_GMRESData *gmres_data = (hypre_GMRESData *)gmres_vdata;
...
HYPRE_Int hypre_GMRESSetSkipRealResidualCheck(void *gmres_vdata,
...
HYPRE_Int hypre_GMRESGetSkipRealResidualCheck(void *gmres_vdata,
                                              HYPRE_Int *skip_real_r_check) {
  hypre_GMRESData *gmres_data = (hypre_GMRESData *)gmres_vdata;
...
HYPRE_Int hypre_GMRESSetPrecond(
    void *gmres_vdata, HYPRE_Int (*precond)(void *, void *, void *, void *),
    HYPRE_Int (*precond_setup)(void *, void *, void *, void *),
    void *precond_data) {
  hypre_GMRESData *gmres_data = (hypre_GMRESData *)gmres_vdata;
  hypre_GMRESFunctions *gmres_functions = gmres_data->functions;
...
HYPRE_Int hypre_GMRESGetPrecond(void *gmres_vdata,
...
HYPRE_Int hypre_GMRESGetNumIterations(void *gmres_vdata,
