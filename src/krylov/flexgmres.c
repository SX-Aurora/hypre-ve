...
hypre_FlexGMRESFunctions *hypre_FlexGMRESFunctionsCreate(
  ...
  fgmres_functions = (hypre_FlexGMRESFunctions *)CAlloc(
      1, sizeof(hypre_FlexGMRESFunctions), HYPRE_MEMORY_HOST);
  ...
  fgmres_functions->CreateVectorArray =
...
void *hypre_FlexGMRESCreate(hypre_FlexGMRESFunctions *fgmres_functions) {
  ...
  if (avals_ptr == NULL) {
   ...
   //  fprintf(stderr, "FGMRES array of pointers has been allocated\n");
  }
#endif
...
HYPRE_Int hypre_FlexGMRESSetup(void *fgmres_vdata, void *A, void *b, void *x) {
  ...
  HYPRE_Int (*precond_setup)(void *, void *, void *, void *) =
      (fgmres_functions->precond_setup);
  ...
  if ((fgmres_data->p) == NULL)
    (fgmres_data->p) =
        (void **)(*(fgmres_functions->CreateVectorArray))(k_dim + 1, x);
  if ((fgmres_data->r) == NULL)
  ...
  if ((fgmres_data->logging) > 0 || (fgmres_data->print_level) > 0) {
    if ((fgmres_data->norms) == NULL)
      (fgmres_data->norms) = hypre_CTAllocF(
          HYPRE_Real, max_iter + 1, fgmres_functions, HYPRE_MEMORY_HOST);
  }
...
HYPRE_Int hypre_FlexGMRESSolve(void *fgmres_vdata, void *A, void *b, void *x) {
  ...
  for (i = 0; i < k_dim + 1; i++) {
    hh[i] =
        hypre_CTAllocF(HYPRE_Real, k_dim, fgmres_functions, HYPRE_MEMORY_HOST);
  }
  ...
  if (ieee_check != ieee_check) {
    ...
    if (logging > 0 || print_level > 0) {
      ...
      hypre_printf("ERROR -- hypre_FlexGMRESSolve: INFs and/or NaNs detected "
                   "in input.\n");
  ...
  while (iter < max_iter) {
    ...
    while (i < k_dim && iter < max_iter) {
      ...
      (*(fgmres_functions->Matvec))(matvec_data, 1.0, A, pre_vecs[i - 1], 0.0,
                                    p[i]);
    ...
    if (i)
      (*(fgmres_functions->Axpy))(rs[i] - 1.0, p[i], p[i]);
    for (j = i - 1; j > 0; j--)
 ...
 *hypre_FlexGMRESGetConvergenceFactorTol
 *--------------------------------------------------------------------------*/

HYPRE_Int hypre_FlexGMRESSetConvergenceFactorTol(void *fgmres_vdata,
                                                 HYPRE_Real cf_tol) {
  hypre_FlexGMRESData *fgmres_data = (hypre_FlexGMRESData *)fgmres_vdata;
...
HYPRE_Int hypre_FlexGMRESGetConvergenceFactorTol(void *fgmres_vdata,
                                                 HYPRE_Real *cf_tol) {
  hypre_FlexGMRESData *fgmres_data = (hypre_FlexGMRESData *)fgmres_vdata;
...
HYPRE_Int hypre_FlexGMRESSetPrecond(
    void *fgmres_vdata, HYPRE_Int (*precond)(void *, void *, void *, void *),
    HYPRE_Int (*precond_setup)(void *, void *, void *, void *),
    void *precond_data) {
  hypre_FlexGMRESData *fgmres_data = (hypre_FlexGMRESData *)fgmres_vdata;
  hypre_FlexGMRESFunctions *fgmres_functions = fgmres_data->functions;
...
HYPRE_Int hypre_FlexGMRESGetPrecond(void *fgmres_vdata,
...
HYPRE_Int hypre_FlexGMRESGetNumIterations(void *fgmres_vdata,
...
HYPRE_Int hypre_FlexGMRESGetConverged(void *fgmres_vdata,
                                      HYPRE_Int *converged) {
  hypre_FlexGMRESData *fgmres_data = (hypre_FlexGMRESData *)fgmres_vdata;
...
HYPRE_Int hypre_FlexGMRESGetFinalRelativeResidualNorm(
    void *fgmres_vdata, HYPRE_Real *relative_residual_norm) {
  hypre_FlexGMRESData *fgmres_data = (hypre_FlexGMRESData *)fgmres_vdata;
...
HYPRE_Int hypre_FlexGMRESModifyPCDefault(void *precond_data,
                                         HYPRE_Int iteration,
                                         HYPRE_Real rel_residual_norm) {
