 ...
 * hypre_OutOfMemory
 *--------------------------------------------------------------------------*/
static inline void hypre_OutOfMemory(size_t size) {
  hypre_error_w_msg(HYPRE_ERROR_MEMORY,
                    "Out of memory trying to allocate too many bytes\n");
  hypre_assert(0);
  fflush(stdout);
}

static inline void hypre_WrongMemoryLocation() {
  hypre_error_w_msg(
      HYPRE_ERROR_MEMORY,
      "Wrong HYPRE MEMORY location: Only HYPRE_MEMORY_HOST, "
      "HYPRE_MEMORY_DEVICE and HYPRE_MEMORY_HOST_PINNED are supported!\n");
  hypre_assert(0);
  fflush(stdout);
}
 ...
 * Memset
 *--------------------------------------------------------------------------*/
static inline void hypre_HostMemset(void *ptr, HYPRE_Int value, size_t num) {
  memset(ptr, value, num);
}

static inline void hypre_DeviceMemset(void *ptr, HYPRE_Int value, size_t num) {
...
#define DEVICE_VAR is_device_ptr(ucptr)
  hypre_LoopBegin(num, k) { ucptr[k] = ucvalue; }
  hypre_LoopEnd()
#undef DEVICE_VAR
...
static inline void hypre_UnifiedMemset(void *ptr, HYPRE_Int value, size_t num) {
 ...
 * Memprefetch
 *--------------------------------------------------------------------------*/
static inline void hypre_UnifiedMemPrefetch(void *ptr, size_t size,
  ...
  if (location == hypre_MEMORY_DEVICE) {
    HYPRE_CUDA_CALL(
        cudaMemPrefetchAsync(ptr, size, hypre_HandleCudaDevice(hypre_handle()),
 ...
 * Malloc
 *--------------------------------------------------------------------------*/
static inline void *hypre_HostMalloc(size_t size, HYPRE_Int zeroinit) {
  void *ptr = NULL;
...
static inline void *hypre_DeviceMalloc(size_t size, HYPRE_Int zeroinit) {
  void *ptr = NULL;
...
static inline void *hypre_UnifiedMalloc(size_t size, HYPRE_Int zeroinit) {
  void *ptr = NULL;
...
#endif
  // HYPRE_CUDA_CALL( cudaMemAdvise(ptr, size,
...
static inline void *hypre_HostPinnedMalloc(size_t size, HYPRE_Int zeroinit) {
  void *ptr = NULL;
...
static inline void *hypre_MAlloc_core(size_t size, HYPRE_Int zeroinit,
                                      hypre_MemoryLocation location) {
  if (size == 0) {
    return NULL;
  }
 ...
 * Free
 *--------------------------------------------------------------------------*/
static inline void hypre_HostFree(void *ptr) { free(ptr); }

static inline void hypre_DeviceFree(void *ptr) {
#if defined(HYPRE_DEVICE_OPENMP_ALLOC)
  omp_target_free(ptr, hypre__offload_device_num);
#elif defined(HYPRE_USING_DEVICE_OPENMP)
  HYPRE_OMPOffload(hypre__offload_device_num, ptr, ((size_t *)ptr)[-1], "exit",
                   "delete");
#elif defined(HYPRE_USING_CUDA)
...
static inline void hypre_UnifiedFree(void *ptr) {
#if defined(HYPRE_USING_CUDA) || defined(HYPRE_USING_DEVICE_OPENMP)
#ifdef HYPRE_USING_CUB_ALLOCATOR
...
static inline void hypre_HostPinnedFree(void *ptr) {
#if defined(HYPRE_USING_CUDA) || defined(HYPRE_USING_DEVICE_OPENMP)
  HYPRE_CUDA_CALL(cudaFreeHost(ptr));
#endif
}

static inline void hypre_Free_core(void *ptr, hypre_MemoryLocation location) {
  if (!ptr) {
    return;
  }
 ...
 * Memcpy
 *--------------------------------------------------------------------------*/
static inline void hypre_Memcpy_core(void *dst, void *src, size_t size,
                                     hypre_MemoryLocation loc_dst,
  ...
  if (loc_dst == hypre_MEMORY_DEVICE &&
      (loc_src == hypre_MEMORY_HOST || loc_src == hypre_MEMORY_HOST_PINNED)) {
#if defined(HYPRE_DEVICE_OPENMP_ALLOC)
    omp_target_memcpy(dst, src, size, 0, 0, hypre__offload_device_num,
                      hypre__offload_host_num);
#elif defined(HYPRE_USING_DEVICE_OPENMP)
    memcpy(dst, src, size);
...
#if defined(HYPRE_DEVICE_OPENMP_ALLOC)
    omp_target_memcpy(dst, src, size, 0, 0, hypre__offload_host_num,
 ...
 *--------------------------------------------------------------------------*/
void *hypre_Memset(void *ptr, HYPRE_Int value, size_t num,
                   HYPRE_MemoryLocation location) {
  if (num == 0) {
    return ptr;
  }
...
void *hypre_CAlloc(size_t count, size_t elt_size,
                   HYPRE_MemoryLocation location) {
  return hypre_MAlloc_core(count * elt_size, 1,
                           hypre_GetActualMemLocation(location));
}
...
void hypre_Memcpy(void *dst, void *src, size_t size,
...
void *hypre_ReAlloc(void *ptr, size_t size, HYPRE_MemoryLocation location) {
  ...
  if (hypre_GetActualMemLocation(location) != hypre_MEMORY_HOST) {
    hypre_printf("hypre_TReAlloc only works with HYPRE_MEMORY_HOST; Use "
                 "hypre_TReAlloc_v2 instead!\n");
    hypre_assert(0);
    hypre_MPI_Abort(hypre_MPI_COMM_WORLD, -1);
    return NULL;
  }
...
void *hypre_ReAlloc_v2(void *ptr, size_t old_size, size_t new_size,
                       HYPRE_MemoryLocation location) {
  if (new_size == 0) {
    hypre_Free(ptr, location);
    return NULL;
 ...
 *--------------------------------------------------------------------------*/
HYPRE_Int hypre_GetPointerLocation(const void *ptr,
...
HYPRE_Int hypre_PrintMemoryTracker() {
  ...
  char *mark =
      hypre_CTAlloc(char, hypre_memory_tracker.size(), HYPRE_MEMORY_HOST);

  for (i = 0; i < hypre_memory_tracker.size(); i++) {
    if (hypre_memory_tracker[i]._ptr == NULL &&
        hypre_memory_tracker[i]._nbytes == 0) {
      continue;
    }

    hypre_fprintf(file, "%6ld: %8s  %16p  %10ld  %d  %32s  %64s      %d\n", i,
                  hypre_memory_tracker[i]._action, hypre_memory_tracker[i]._ptr,
    ...
    if (strstr(hypre_memory_tracker[i]._action, "alloc") != NULL) {
      ...
      for (j = i + 1; j < hypre_memory_tracker.size(); j++) {
        if (mark[j] == 0 &&
            ...
            hypre_memory_tracker[i]._memory_location ==
                hypre_memory_tracker[j]._memory_location) {
          mark[j] = 1;
          found = 1;
          break;
        }
...
HYPRE_Int hypre_SetCubMemPoolSize(hypre_uint cub_bin_growth,
  ...
  if (hypre_HandleCubDevAllocator(hypre_handle())) {
    hypre_HandleCubDevAllocator(hypre_handle())
        ->SetMaxCachedBytes(cub_max_cached_bytes);
  }
...
cudaError_t hypre_CachingMallocDevice(void **ptr, size_t nbytes) {
  if (!hypre_HandleCubDevAllocator(hypre_handle())) {
    hypre_HandleCubDevAllocator(hypre_handle()) =
        new hypre_cub_CachingDeviceAllocator(
            hypre_HandleCubBinGrowth(hypre_handle()),
            hypre_HandleCubMinBin(hypre_handle()),
            hypre_HandleCubMaxBin(hypre_handle()),
            hypre_HandleCubMaxCachedBytes(hypre_handle()), false, false, false);
  }

  return hypre_HandleCubDevAllocator(hypre_handle())
      ->DeviceAllocate(ptr, nbytes);
}
  ...
  if (!hypre_HandleCubUvmAllocator(hypre_handle())) {
    hypre_HandleCubUvmAllocator(hypre_handle()) =
        new hypre_cub_CachingDeviceAllocator(
            ...
            hypre_HandleCubMinBin(hypre_handle()),
            hypre_HandleCubMaxBin(hypre_handle()),
            hypre_HandleCubMaxCachedBytes(hypre_handle()), false, false, true);
  }

  return hypre_HandleCubUvmAllocator(hypre_handle())
      ->DeviceAllocate(ptr, nbytes);
}
