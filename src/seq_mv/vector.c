...
hypre_Vector *hypre_SeqMultiVectorCreate(HYPRE_Int size,
...
HYPRE_Int hypre_SeqVectorInitialize_v2(hypre_Vector *vector,
  ...
  if (!hypre_VectorData(vector)) {
    hypre_VectorData(vector) =
        hypre_CTAlloc(HYPRE_Complex, num_vectors * size, memory_location);
  }
...
HYPRE_Int hypre_SeqVectorInitialize(hypre_Vector *vector) {
  ...
  ierr =
      hypre_SeqVectorInitialize_v2(vector, hypre_VectorMemoryLocation(vector));
...
HYPRE_Int hypre_SeqVectorSetDataOwner(hypre_Vector *vector,
                                      HYPRE_Int owns_data) {
  HYPRE_Int ierr = 0;
...
HYPRE_Int hypre_SeqVectorSetConstantValues(hypre_Vector *v,
                                           HYPRE_Complex value) {
#ifdef HYPRE_PROFILE
...
HYPRE_Int hypre_SeqVectorSetRandomValues(hypre_Vector *v, HYPRE_Int seed) {
  ...
  if (hypre_GetActualMemLocation(hypre_VectorMemoryLocation(v)) ==
      hypre_MEMORY_HOST) {
    ...
    HYPRE_Complex *h_data =
        hypre_TAlloc(HYPRE_Complex, size, HYPRE_MEMORY_HOST);
    for (i = 0; i < size; i++) {
      h_data[i] = 2.0 * hypre_Rand() - 1.0;
    }
    hypre_TMemcpy(vector_data, h_data, HYPRE_Complex, size,
                  hypre_VectorMemoryLocation(v), HYPRE_MEMORY_HOST);
...
HYPRE_Int hypre_SeqVectorCopy(hypre_Vector *x, hypre_Vector *y) {
  ...
  size_t size = hypre_min(hypre_VectorSize(x), hypre_VectorSize(y)) *
                hypre_VectorNumVectors(x);

  hypre_TMemcpy(hypre_VectorData(y), hypre_VectorData(x), HYPRE_Complex, size,
...
hypre_Vector *
hypre_SeqVectorCloneDeep_v2(hypre_Vector *x,
                            HYPRE_MemoryLocation memory_location) {
...
HYPRE_Int hypre_SeqVectorScale(HYPRE_Complex alpha, hypre_Vector *y) {
  ...
  HYPRE_CUBLAS_CALL(cublasDscal(hypre_HandleCublasHandle(hypre_handle()), size,
                                &alpha, y_data, 1));
#else
 ...
 * hypre_SeqVectorAxpy
 *--------------------------------------------------------------------------*/
HYPRE_Int hypre_SeqVectorAxpy(HYPRE_Complex alpha, hypre_Vector *x,
                              hypre_Vector *y) {
#ifdef HYPRE_PROFILE
  hypre_profile_times[HYPRE_TIMER_ID_BLAS1] -= hypre_MPI_Wtime();
#endif
...
#if defined(HYPRE_USING_CUDA)
#if defined(HYPRE_USING_CUBLAS)
  HYPRE_CUBLAS_CALL(cublasDaxpy(hypre_HandleCublasHandle(hypre_handle()), size,
                                &alpha, x_data, 1, y_data, 1));
#else
  HYPRE_THRUST_CALL(transform, x_data, x_data + size, y_data, y_data,
                    alpha * _1 + _2);
#endif
#else
  HYPRE_Int i;
...
#if defined(HYPRE_USING_CUBLAS)
  HYPRE_CUBLAS_CALL(cublasDdot(hypre_HandleCublasHandle(hypre_handle()), size,
...
HYPRE_Int hypre_SeqVectorPrefetch(hypre_Vector *x,
                                  HYPRE_MemoryLocation memory_location) {
  HYPRE_Int ierr = 0;
#ifdef HYPRE_USING_UNIFIED_MEMORY
  if (hypre_VectorMemoryLocation(x) != HYPRE_MEMORY_DEVICE) {
    /* hypre_error_w_msg(HYPRE_ERROR_GENERIC," Error! CUDA Prefetch with
     * non-unified momory\n");*/
    return 1;
  }
