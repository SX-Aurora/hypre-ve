...
/*
   ...
   Description:  This example solves the 2-D Laplacian problem with zero
   boundary conditions on an n x n grid.  The number of unknowns is N=n^2. The
   standard 5-point stencil is used, and we solve for the interior nodes only.
...
int main(int argc, char *argv[]) {
  ...
  /* Parse command line */
  {
    ...
    if ((print_usage) && (myid == 0)) {
      ...
      printf("  -vis                : save the solution for GLVis "
             "visualization\n");
  ...
  */
  ...
  /* GMRES*/
    ...
    HYPRE_ParCSRGMRESSetTol(solver, 1e-7);

    HYPRE_ParCSRGMRESSetPrintLevel(solver, 1);
