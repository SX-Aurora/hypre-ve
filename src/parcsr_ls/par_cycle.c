...
HYPRE_Int hypre_BoomerAMGCycle(void *amg_vdata, hypre_ParVector **F_array,
  ...
  partial_cycle_coarsest_level =
      hypre_ParAMGDataPartialCycleCoarsestLevel(amg_data);
  ...
  if (smooth_num_levels > 0) {
    if (smooth_type == 7 || smooth_type == 8 || smooth_type == 17 ||
        smooth_type == 18 || smooth_type == 9 || smooth_type == 19) {
  ...
  while (Not_Finished) {
    if (num_levels > 1) {
      ...
      if (smooth_num_levels <= level) {
        ...
        hypre_ParCSRMatrixMatvecOutOfPlace(
            alpha, A_array[level], U_array[level], beta, F_array[level], Rtemp);
    ...
    } else /* AB: 4/08: removed the max_levels > 1 check - should do this when
              max-levels = 1 also */
    {
      ...
      for (jj = 0; jj < cg_num_sweep; jj++) {
        ...
        for (j = 0; j < num_sweep; j++) {
          ...
          if (smooth_num_levels > level &&
            ...
            hypre_ParCSRMatrixMatvecOutOfPlace(
                alpha, A_array[level], U_array[level], beta, Aux_F, Vtemp);
            if (smooth_type == 8 || smooth_type == 18) {
              HYPRE_ParCSRParaSailsSolve(
                  smoother[level], (HYPRE_ParCSRMatrix)A_array[level],
                  (HYPRE_ParVector)Vtemp, (HYPRE_ParVector)Utemp);
            ...
            HYPRE_ILUSolve(smoother[level], (HYPRE_ParCSRMatrix)A_array[level],
            ...
            HYPRE_SchwarzSolve(smoother[level],
                     ...
                     relax_type == 199) { /* Gaussian elimination */
            hypre_GaussElimSolve(amg_data, level, relax_type);
          } else if (relax_type == 18) { /* L1 - Jacobi*/
            if (relax_order == 1 && cycle_param < 3) {
              ...
              for (i = 0; i < 2; i++) {
                hypre_ParCSRRelax_L1_Jacobi(
                    A_array[level], Aux_F, CF_marker_array[level],
                    loc_relax_points[i], relax_weight[level],
            ...
            } else /* not CF - so use through AMS */
            {
              ...
              hypre_ParCSRRelax(
                  A_array[level], Aux_F, 1, 1,
                  l1_norms_level ? hypre_VectorData(l1_norms_level) : NULL,
                  relax_weight[level], omega[level], 0, 0, 0, 0, Aux_U, Vtemp,
                  Ztemp);
#else
              if (hypre_NumThreads() == 1) {
                ...
                hypre_ParCSRRelaxThreads(
                    A_array[level], Aux_F, 1, 1,
                    l1_norms_level ? hypre_VectorData(l1_norms_level) : NULL,
                    relax_weight[level], omega[level], Aux_U, Vtemp, Ztemp);
              }
#endif
            }
          } else if (relax_type == 15) { /* CG */
            if (j == 0)                  /* do num sweep iterations of CG */
            {
              hypre_ParCSRRelax_CG(smoother[level], A_array[level], Aux_F,
                                   Aux_U, num_sweep);
            }
          } else if (relax_type == 16) { /* scaled Chebyshev */
            HYPRE_Int scale = hypre_ParAMGDataChebyScale(amg_data);
            HYPRE_Int variant = hypre_ParAMGDataChebyVariant(amg_data);
            hypre_ParCSRRelax_Cheby_Solve(A_array[level], Aux_F, ds[level],
                                          coefs[level], cheby_order, scale,
                                          variant, Aux_U, Vtemp, Ztemp);
          } else if (relax_type == 17) {
            // printf("Proc %d: level %d, n %d, CF %p\n", my_id, level,
            // local_size, CF_marker_array[level]);
            if (level == num_levels - 1) {
              ...
              hypre_BoomerAMGRelax(
                  A_array[level], Aux_F, CF_marker_array[level], 0, 0,
                  relax_weight[level], 0.0, NULL, Aux_U, Vtemp, NULL);
            } else {
              hypre_BoomerAMGRelax_FCFJacobi(A_array[level], Aux_F,
                                             CF_marker_array[level],
                                             relax_weight[level], Aux_U, Vtemp);
            }
          } else if (old_version) {
            /*
               ...
               */
            Solve_err_flag = hypre_BoomerAMGRelax(
                A_array[level], Aux_F, CF_marker_array[level], relax_type,
                relax_points, relax_weight[level], omega[level],
                l1_norms_level ? hypre_VectorData(l1_norms_level) : NULL, Aux_U,
                Vtemp, Ztemp);
          } else {
            ...
            if (block_mode) {
              Solve_err_flag = hypre_BoomerAMGBlockRelaxIF(
                  ...
                  relax_type, relax_local, cycle_param, relax_weight[level],
                  omega[level], Aux_U, Vtemp);
            } else {
              Solve_err_flag = hypre_BoomerAMGRelaxIF(
                  A_array[level], Aux_F, CF_marker_array[level], relax_type,
                  relax_local, cycle_param, relax_weight[level], omega[level],
                  l1_norms_level ? hypre_VectorData(l1_norms_level) : NULL,
                  Aux_U, Vtemp, Ztemp);
            }
          }
    ...
    if (lev_counter[level] >= 0 && level != num_levels - 1) {
      ...
      if (block_mode) {
        ...
        hypre_ParCSRBlockMatrixMatvec(alpha, A_block_array[fine_grid],
                                      U_array[fine_grid], beta, Vtemp);
      } else {
        // JSP: avoid unnecessary copy using out-of-place version of SpMV
        hypre_ParCSRMatrixMatvecOutOfPlace(alpha, A_array[fine_grid],
      ...
      if (block_mode) {
        ...
        if (restri_type) {
          ...
          hypre_ParCSRMatrixMatvec(alpha, R_array[fine_grid], Vtemp, beta,
                                   F_array[coarse_grid]);
        } else {
          hypre_ParCSRMatrixMatvecT(alpha, R_array[fine_grid], Vtemp, beta,
                                    F_array[coarse_grid]);
        }
      }
      ...
      if (partial_cycle_coarsest_level >= 0 &&
          level == partial_cycle_coarsest_level + 1) {
        Not_Finished = 0;
      }
