...
HYPRE_Int hypre_IJMatrixCreateParCSR(hypre_IJMatrix *matrix) {
  ...
  if (hypre_IJMatrixGlobalFirstRow(matrix)) {
    for (i = 0; i < 2; i++) {
      row_starts[i] =
          row_partitioning[i] - hypre_IJMatrixGlobalFirstRow(matrix);
    }
  } else {
    for (i = 0; i < 2; i++) {
  ...
  if (row_partitioning != col_partitioning) {
    ...
    if (hypre_IJMatrixGlobalFirstCol(matrix)) {
      for (i = 0; i < 2; i++) {
        col_starts[i] =
            col_partitioning[i] - hypre_IJMatrixGlobalFirstCol(matrix);
      }
    } else {
      for (i = 0; i < 2; i++) {
  ...
  par_matrix = hypre_ParCSRMatrixCreate(
      comm, hypre_IJMatrixGlobalNumRows(matrix),
...
HYPRE_Int hypre_IJMatrixSetRowSizesParCSR(hypre_IJMatrix *matrix,
  ...
  local_num_rows =
      (HYPRE_Int)(row_partitioning[my_id + 1] - row_partitioning[my_id]);
  local_num_cols =
      (HYPRE_Int)(col_partitioning[my_id + 1] - col_partitioning[my_id]);
#endif
  hypre_AuxParCSRMatrix *aux_matrix =
      (hypre_AuxParCSRMatrix *)hypre_IJMatrixTranslator(matrix);
  ...
  if (!aux_matrix) {
    hypre_AuxParCSRMatrixCreate(&aux_matrix, local_num_rows, local_num_cols,
                                row_space);
...
HYPRE_Int hypre_IJMatrixSetDiagOffdSizesParCSR(hypre_IJMatrix *matrix,
  ...
  if (hypre_AuxParCSRMatrixDiagSizes(aux_matrix) == NULL) {
    hypre_AuxParCSRMatrixDiagSizes(aux_matrix) =
        hypre_TAlloc(HYPRE_Int, local_num_rows, HYPRE_MEMORY_HOST);
  }

  if (hypre_AuxParCSRMatrixOffdSizes(aux_matrix) == NULL) {
    hypre_AuxParCSRMatrixOffdSizes(aux_matrix) =
        hypre_TAlloc(HYPRE_Int, local_num_rows, HYPRE_MEMORY_HOST);
  }

  hypre_TMemcpy(hypre_AuxParCSRMatrixDiagSizes(aux_matrix), diag_sizes,
                HYPRE_Int, local_num_rows, HYPRE_MEMORY_HOST,
                HYPRE_MEMORY_HOST);

  hypre_TMemcpy(hypre_AuxParCSRMatrixOffdSizes(aux_matrix), offd_sizes,
                HYPRE_Int, local_num_rows, HYPRE_MEMORY_HOST,
                HYPRE_MEMORY_HOST);
...
HYPRE_Int hypre_IJMatrixSetMaxOnProcElmtsParCSR(hypre_IJMatrix *matrix,
...
HYPRE_Int hypre_IJMatrixInitializeParCSR(hypre_IJMatrix *matrix) {
  return hypre_IJMatrixInitializeParCSR_v2(
      matrix, hypre_HandleMemoryLocation(hypre_handle()));
}

HYPRE_Int
hypre_IJMatrixInitializeParCSR_v2(hypre_IJMatrix *matrix,
                                  HYPRE_MemoryLocation memory_location) {
  hypre_ParCSRMatrix *par_matrix =
      (hypre_ParCSRMatrix *)hypre_IJMatrixObject(matrix);
  hypre_AuxParCSRMatrix *aux_matrix =
      (hypre_AuxParCSRMatrix *)hypre_IJMatrixTranslator(matrix);

  HYPRE_MemoryLocation memory_location_aux =
      hypre_GetExecPolicy1(memory_location) == HYPRE_EXEC_HOST
          ? HYPRE_MEMORY_HOST
          : HYPRE_MEMORY_DEVICE;
...
#pragma omp parallel for private(i) HYPRE_SMP_SCHEDULE
#endif
      for (i = 0; i < local_num_rows; i++) {
        hypre_AuxParCSRMatrixIndxDiag(aux_matrix)[i] =
            hypre_CSRMatrixI(diag)[i];
        hypre_AuxParCSRMatrixIndxOffd(aux_matrix)[i] =
            hypre_CSRMatrixI(offd)[i];
      }
    }
    ...
    */
    if (!aux_matrix) {
      hypre_AuxParCSRMatrixCreate(&aux_matrix,
                                  hypre_ParCSRMatrixNumRows(par_matrix),
                                  hypre_ParCSRMatrixNumCols(par_matrix), NULL);
...
HYPRE_Int hypre_IJMatrixGetRowCountsParCSR(hypre_IJMatrix *matrix,
                                           HYPRE_Int nrows, HYPRE_BigInt *rows,
                                           HYPRE_Int *ncols) {
...
#pragma omp parallel for private(i, row_index) HYPRE_SMP_SCHEDULE
  ...
  for (i = 0; i < nrows; i++) {
    ...
    if (row_index >= row_partitioning[pstart] &&
      ...
      ncols[i] =
          diag_i[index + 1] - diag_i[index] + offd_i[index + 1] - offd_i[index];
    } else {
      ncols[i] = 0;
      if (print_level) {
...
HYPRE_Int hypre_IJMatrixGetValuesParCSR(hypre_IJMatrix *matrix, HYPRE_Int nrows,
  ...
  if (assemble_flag == 0) {
    ...
    if (print_level) {
      hypre_printf(
          "Error! Matrix not assembled yet! HYPRE_IJMatrixGetValues\n");
    }
  }
  ...
  if (nrows < 0) {
    ...
    for (i = 0; i < nrows; i++) {
      row = rows[i];
      if (row >= row_partitioning[pstart] &&
          row < row_partitioning[pstart + 1]) {
...
HYPRE_Int hypre_IJMatrixSetValuesParCSR(hypre_IJMatrix *matrix, HYPRE_Int nrows,
                                        HYPRE_Int *ncols,
    ...
    for (ii = 0; ii < nrows; ii++) {
      ...
      if (row >= row_partitioning[pstart] &&
        ...
        if (need_aux) {
          ...
          if (tmp_indx) {
            aux_j[row_local] =
                hypre_TReAlloc(aux_j[row_local], HYPRE_BigInt, size + tmp_indx,
                               HYPRE_MEMORY_HOST);
            aux_data[row_local] =
                hypre_TReAlloc(aux_data[row_local], HYPRE_Complex,
...
void hypre_IJMatrixSetConstantValuesParCSRHost(hypre_IJMatrix *matrix,
                                               HYPRE_Complex value) {
  hypre_ParCSRMatrix *par_matrix =
      (hypre_ParCSRMatrix *)hypre_IJMatrixObject(matrix);
  hypre_CSRMatrix *diag = hypre_ParCSRMatrixDiag(par_matrix);
...
HYPRE_Int hypre_IJMatrixSetConstantValuesParCSR(hypre_IJMatrix *matrix,
                                                HYPRE_Complex value) {
  if (hypre_IJMatrixAssembleFlag(matrix)) /* matrix already assembled*/
  {
#if defined(HYPRE_USING_CUDA)
    if (hypre_GetExecPolicy1(hypre_IJMatrixMemoryLocation(matrix)) ==
        HYPRE_EXEC_DEVICE) {
...
HYPRE_Int hypre_IJMatrixAddToValuesParCSR(hypre_IJMatrix *matrix,
                                          HYPRE_Int nrows, HYPRE_Int *ncols,
                                          const HYPRE_BigInt *rows,
                                          const HYPRE_Int *row_indexes,
                                          const HYPRE_BigInt *cols,
                                          const HYPRE_Complex *values) {
  ...
  if (hypre_IJMatrixAssembleFlag(matrix)) {
    ...
    for (ii = 0; ii < nrows; ii++) {
      ...
      /* not my row */
        ...
        if (!aux_matrix) {
          size = (HYPRE_Int)(row_partitioning[pstart + 1] -
                             row_partitioning[pstart]);
        ...
        if (!max_off_proc_elmts) {
          ...
          hypre_AuxParCSRMatrixMaxOffProcElmts(aux_matrix) = max_off_proc_elmts;
          hypre_AuxParCSRMatrixOffProcI(aux_matrix) = hypre_CTAlloc(
              HYPRE_BigInt, 2 * max_off_proc_elmts, HYPRE_MEMORY_HOST);
          hypre_AuxParCSRMatrixOffProcJ(aux_matrix) = hypre_CTAlloc(
              HYPRE_BigInt, max_off_proc_elmts, HYPRE_MEMORY_HOST);
          hypre_AuxParCSRMatrixOffProcData(aux_matrix) = hypre_CTAlloc(
              HYPRE_Complex, max_off_proc_elmts, HYPRE_MEMORY_HOST);
          off_proc_i = hypre_AuxParCSRMatrixOffProcI(aux_matrix);
          off_proc_j = hypre_AuxParCSRMatrixOffProcJ(aux_matrix);
          off_proc_data = hypre_AuxParCSRMatrixOffProcData(aux_matrix);
          ...
          off_proc_i =
              hypre_TReAlloc(off_proc_i, HYPRE_BigInt, 2 * max_off_proc_elmts,
                             HYPRE_MEMORY_HOST);
          off_proc_j = hypre_TReAlloc(off_proc_j, HYPRE_BigInt,
...
HYPRE_Int hypre_IJMatrixAssembleOffProcValsParCSR(
    hypre_IJMatrix *matrix, HYPRE_Int off_proc_i_indx,
    HYPRE_Int max_off_proc_elmts, HYPRE_Int current_num_elmts,
    HYPRE_MemoryLocation memory_location, HYPRE_BigInt *off_proc_i,
    HYPRE_BigInt *off_proc_j, HYPRE_Complex *off_proc_data) {
  MPI_Comm comm = hypre_IJMatrixComm(matrix);
  ...
  proc_id_mem =
      hypre_CTAlloc(HYPRE_Int, off_proc_i_indx / 2, HYPRE_MEMORY_HOST);
  j = 0;
  ...
  hypre_MPI_Allgather(&num_sends3, 1, HYPRE_MPI_INT, info, 1, HYPRE_MPI_INT,
                      comm);
  ...
  hypre_MPI_Allgatherv(int_buffer, num_sends3, HYPRE_MPI_INT, recv_buf, info,
  ...
  for (i = 0; i < num_procs; i++) {
    for (j = displs[i]; j < displs[i + 1]; j += 3) {
      if (recv_buf[j] == my_id) {
        ...
        recv_vec_starts[j2] =
            recv_vec_starts[j2 - 1] + 2 * recv_buf[j + 1] + recv_buf[j + 2];
  ...
  for (i = 0; i < num_recvs; i++) {
    ...
    ip = recv_procs[i];
    hypre_MPI_Irecv(&recv_data[vec_start], vec_len, HYPRE_MPI_COMPLEX, ip, 0,
                    comm, &requests[j++]);
  }
  ...
  for (i = 0; i < num_sends; i++) {
    ...
    ip = send_procs[i];
    hypre_MPI_Isend(&send_data[vec_start], vec_len, HYPRE_MPI_COMPLEX, ip, 0,
                    comm, &requests[j++]);
  }
...
HYPRE_Int hypre_IJMatrixAssembleOffProcValsParCSR(
  ...
  if (memory_location == HYPRE_MEMORY_DEVICE) {
    HYPRE_BigInt *tmp =
        hypre_TAlloc(HYPRE_BigInt, current_num_elmts, HYPRE_MEMORY_HOST);
    HYPRE_BigInt *off_proc_i_h =
        hypre_TAlloc(HYPRE_BigInt, 2 * current_num_elmts, HYPRE_MEMORY_HOST);
    HYPRE_BigInt *off_proc_j_h =
        hypre_TAlloc(HYPRE_BigInt, current_num_elmts, HYPRE_MEMORY_HOST);
    HYPRE_Complex *off_proc_data_h =
        hypre_TAlloc(HYPRE_Complex, current_num_elmts, HYPRE_MEMORY_HOST);
    ...
    hypre_TMemcpy(off_proc_j_h, off_proc_j, HYPRE_BigInt, current_num_elmts,
                  HYPRE_MEMORY_HOST, HYPRE_MEMORY_DEVICE);
    hypre_TMemcpy(off_proc_data_h, off_proc_data, HYPRE_Complex,
                  current_num_elmts, HYPRE_MEMORY_HOST, HYPRE_MEMORY_DEVICE);
  ...
  ex_contact_procs =
      hypre_CTAlloc(HYPRE_Int, ex_num_contacts, HYPRE_MEMORY_HOST);
  ex_contact_vec_starts =
      hypre_CTAlloc(HYPRE_Int, ex_num_contacts + 1, HYPRE_MEMORY_HOST);
  ex_contact_buf =
      hypre_CTAlloc(HYPRE_BigInt, ex_num_contacts * 2, HYPRE_MEMORY_HOST);
  ...
  for (i = 0; i < num_rows; i++) {
    if (row_list[i] > range_end) {
      ...
      hypre_GetAssumedPartitionRowRange(comm, proc_id, global_first_col,
                                        global_num_cols, &range_start,
                                        &range_end);
    }
  }
  /* finish the starts */
  ...
  hypre_DataExchangeList(
      ...
      max_response_size, 1, comm, (void **)&response_buf, &response_buf_starts);
  ...
  ex_contact_procs =
      hypre_CTAlloc(HYPRE_Int, num_real_procs, HYPRE_MEMORY_HOST);
  num_rows_per_proc =
      hypre_CTAlloc(HYPRE_Int, num_real_procs, HYPRE_MEMORY_HOST);
  num_elements_total =
      hypre_CTAlloc(HYPRE_Int, num_real_procs, HYPRE_MEMORY_HOST);
  ...
  storage = 0;
  ex_contact_vec_starts =
      hypre_CTAlloc(HYPRE_Int, num_real_procs + 1, HYPRE_MEMORY_HOST);
  ex_contact_vec_starts[0] = -1;

  for (i = 0; i < num_real_procs; i++) {
    storage += 1 + 2 * num_rows_per_proc[i] + 2 * num_elements_total[i];
    ex_contact_vec_starts[i + 1] =
        -storage - 1; /* need negative for next loop */
  }
  ...
  void_contact_buf =
      hypre_CTAlloc(char, storage *obj_size_bytes, HYPRE_MEMORY_HOST);
  ...
  for (i = 0; i < num_rows; i++) {
    ...
    if (in_i < 0) {
      ...
      hypre_TMemcpy(index_ptr, &tmp_int, HYPRE_Int, 1, HYPRE_MEMORY_HOST,
                    HYPRE_MEMORY_HOST);
    ...
    /* add row # */
    hypre_TMemcpy(index_ptr, &row, HYPRE_BigInt, 1, HYPRE_MEMORY_HOST,
                  HYPRE_MEMORY_HOST);
    index_ptr = (void *)((char *)index_ptr + obj_size_bytes);
    in_i++;

    /* add number of elements */
    hypre_TMemcpy(index_ptr, &num_elements, HYPRE_Int, 1, HYPRE_MEMORY_HOST,
                  HYPRE_MEMORY_HOST);
    index_ptr = (void *)((char *)index_ptr + obj_size_bytes);
    in_i++;
    ...
    for (j = 0; j < num_elements; j++) {
      ...
      hypre_TMemcpy(index_ptr, &tmp_big_int, HYPRE_BigInt, 1, HYPRE_MEMORY_HOST,
                    HYPRE_MEMORY_HOST);
      index_ptr = (void *)((char *)index_ptr + obj_size_bytes);
      in_i++;
    }
  ...
  send_proc_obj.id = hypre_CTAlloc(HYPRE_Int, send_proc_obj.storage_length + 1,
                                   HYPRE_MEMORY_HOST);
  send_proc_obj.vec_starts = hypre_CTAlloc(
  ...
  send_proc_obj.v_elements =
      hypre_TAlloc(char, obj_size_bytes *send_proc_obj.element_storage_length,
                   HYPRE_MEMORY_HOST);
  ...
  hypre_DataExchangeList(num_real_procs, ex_contact_procs, void_contact_buf,
                         ex_contact_vec_starts, obj_size_bytes, 0,
                         &response_obj2, max_response_size, 2, comm,
  ...
  argsort_contact_procs =
      hypre_CTAlloc(HYPRE_Int, num_recvs, HYPRE_MEMORY_HOST);
  for (i = 0; i < num_recvs; i++) {
  ...
  for (i = 0; i < num_recvs; i++) {
    ...
    /* Find the current processor in order, and reset recv_data_ptr to that
     * processor's message */
    original_proc_indx = argsort_contact_procs[i];
    /*current_proc = send_proc_obj.id[i];*/
    indx = recv_starts[original_proc_indx];
    recv_data_ptr =
        (void *)((char *)send_proc_obj.v_elements + indx * obj_size_bytes);

    /* get the number of rows for this recv */
    hypre_TMemcpy(&num_rows, recv_data_ptr, HYPRE_Int, 1, HYPRE_MEMORY_HOST,
                  HYPRE_MEMORY_HOST);
    ...
    for (j = 0; j < num_rows; j++) /* for each row: unpack info */
    {
      ...
      hypre_TMemcpy(&num_elements, recv_data_ptr, HYPRE_Int, 1,
                    HYPRE_MEMORY_HOST, HYPRE_MEMORY_HOST);
      recv_data_ptr = (void *)((char *)recv_data_ptr + obj_size_bytes);
      indx++;
      ...
      } else /* copy data */
      {
        if (big_int_data_size < num_elements) {
          big_int_data = hypre_TReAlloc(big_int_data, HYPRE_BigInt,
                                        num_elements + 10, HYPRE_MEMORY_HOST);
        }
        for (k = 0; k < num_elements; k++) {
          hypre_TMemcpy(&big_int_data[k], recv_data_ptr, HYPRE_BigInt, 1,
                        HYPRE_MEMORY_HOST, HYPRE_MEMORY_HOST);
          recv_data_ptr = (void *)((char *)recv_data_ptr + obj_size_bytes);
        }
      ...
      } else /* copy data */
      {
        if (complex_data_size < num_elements) {
          complex_data = hypre_TReAlloc(complex_data, HYPRE_Complex,
                                        num_elements + 10, HYPRE_MEMORY_HOST);
        }
        for (k = 0; k < num_elements; k++) {
          hypre_TMemcpy(&complex_data[k], recv_data_ptr, HYPRE_Complex, 1,
                        HYPRE_MEMORY_HOST, HYPRE_MEMORY_HOST);
          recv_data_ptr = (void *)((char *)recv_data_ptr + obj_size_bytes);
        }
      ...
      if (memory_location == HYPRE_MEMORY_HOST) {
        hypre_IJMatrixAddToValuesParCSR(matrix, 1, &num_elements, &row,
                                        &row_index, col_ptr, col_data_ptr);
      } else {
        ...
        if (nelm_new > off_proc_nelm_recv_max) {
          ...
          off_proc_i_recv =
              hypre_TReAlloc(off_proc_i_recv, HYPRE_BigInt,
                             off_proc_nelm_recv_max, HYPRE_MEMORY_HOST);
          off_proc_j_recv =
              hypre_TReAlloc(off_proc_j_recv, HYPRE_BigInt,
                             off_proc_nelm_recv_max, HYPRE_MEMORY_HOST);
          off_proc_data_recv =
              hypre_TReAlloc(off_proc_data_recv, HYPRE_Complex,
                             off_proc_nelm_recv_max, HYPRE_MEMORY_HOST);
        }
        ...
        hypre_TMemcpy(off_proc_j_recv + off_proc_nelm_recv_cur, col_ptr,
                      HYPRE_BigInt, num_elements, HYPRE_MEMORY_HOST,
                      HYPRE_MEMORY_HOST);
        hypre_TMemcpy(off_proc_data_recv + off_proc_nelm_recv_cur, col_data_ptr,
                      HYPRE_Complex, num_elements, HYPRE_MEMORY_HOST,
                      HYPRE_MEMORY_HOST);
  ...
  if (memory_location == HYPRE_MEMORY_DEVICE) {
    off_proc_i_recv_d =
        hypre_TAlloc(HYPRE_BigInt, off_proc_nelm_recv_cur, HYPRE_MEMORY_DEVICE);
    off_proc_j_recv_d =
        hypre_TAlloc(HYPRE_BigInt, off_proc_nelm_recv_cur, HYPRE_MEMORY_DEVICE);
    off_proc_data_recv_d = hypre_TAlloc(HYPRE_Complex, off_proc_nelm_recv_cur,
                                        HYPRE_MEMORY_DEVICE);

    hypre_TMemcpy(off_proc_i_recv_d, off_proc_i_recv, HYPRE_BigInt,
                  off_proc_nelm_recv_cur, HYPRE_MEMORY_DEVICE,
                  HYPRE_MEMORY_HOST);
    hypre_TMemcpy(off_proc_j_recv_d, off_proc_j_recv, HYPRE_BigInt,
                  off_proc_nelm_recv_cur, HYPRE_MEMORY_DEVICE,
                  HYPRE_MEMORY_HOST);
    hypre_TMemcpy(off_proc_data_recv_d, off_proc_data_recv, HYPRE_Complex,
                  off_proc_nelm_recv_cur, HYPRE_MEMORY_DEVICE,
                  HYPRE_MEMORY_HOST);

#if defined(HYPRE_USING_CUDA)
    hypre_IJMatrixSetAddValuesParCSRDevice(
        matrix, off_proc_nelm_recv_cur, NULL, off_proc_i_recv_d, NULL,
        off_proc_j_recv_d, off_proc_data_recv_d, "add");
#endif
  }
...
HYPRE_Int hypre_FillResponseIJOffProcVals(void *p_recv_contact_buf,
                                          HYPRE_Int contact_size,
                                          HYPRE_Int contact_proc, void *ro,
                                          MPI_Comm comm,
...

{
  ...
  hypre_ProcListElements *send_proc_obj =
      (hypre_ProcListElements *)response_obj->data2;

  object_size = hypre_max(sizeof(HYPRE_BigInt), sizeof(HYPRE_Complex));

  hypre_MPI_Comm_rank(comm, &myid);

  /*check to see if we need to allocate more space in send_proc_obj for vec
   * starts and id */
  if (send_proc_obj->length == send_proc_obj->storage_length) {
    ...
    if (send_proc_obj->id != NULL) {
      send_proc_obj->id =
          hypre_TReAlloc(send_proc_obj->id, HYPRE_Int,
                         send_proc_obj->storage_length + 1, HYPRE_MEMORY_HOST);
    }
  }
  ...
  if (send_proc_obj->element_storage_length < index + contact_size) {
    ...
    elength += index;
    send_proc_obj->v_elements =
        hypre_TReAlloc((char *)send_proc_obj->v_elements, char,
  ...
  hypre_TMemcpy(index_ptr, p_recv_contact_buf, char, object_size *contact_size,
                HYPRE_MEMORY_HOST, HYPRE_MEMORY_HOST);
...
HYPRE_Int hypre_FindProc(HYPRE_BigInt *list, HYPRE_BigInt value,
...
HYPRE_Int hypre_IJMatrixAssembleParCSR(hypre_IJMatrix *matrix) {
  ...
  hypre_MPI_Allreduce(&aux_flag, &aux_flag_global, 1, HYPRE_MPI_INT,
                      hypre_MPI_SUM, comm);
  ...
  if (aux_matrix) {
    ...
    if (offd_proc_elmts) {
      ...
      hypre_IJMatrixAssembleOffProcValsParCSR(
          ...
          HYPRE_MEMORY_HOST, off_proc_i, off_proc_j, off_proc_data);
    }
  }
...
#pragma omp barrier
        ...
        if (my_thread_num == 0) {
          ...
          hypre_TFree(hypre_CSRMatrixJ(diag),
                      hypre_CSRMatrixMemoryLocation(diag));
          ...
          hypre_TFree(hypre_CSRMatrixData(offd),
                      hypre_CSRMatrixMemoryLocation(offd));
          hypre_TFree(hypre_CSRMatrixBigJ(offd),
                      hypre_CSRMatrixMemoryLocation(offd));
          ...
          offd_data = hypre_CTAlloc(HYPRE_Complex, i_offd,
                                    hypre_CSRMatrixMemoryLocation(offd));
          big_offd_j = hypre_CTAlloc(HYPRE_BigInt, i_offd,
                                     hypre_CSRMatrixMemoryLocation(offd));
        }
#ifdef HYPRE_USING_OPENMP
#pragma omp barrier
...
#pragma omp parallel for private(i, j, j0, temp)
      ...
      if (!offd_j && offd_i[num_rows]) {
        offd_j = hypre_CTAlloc(HYPRE_Int, offd_i[num_rows],
                               hypre_CSRMatrixMemoryLocation(offd));
        hypre_CSRMatrixJ(offd) = offd_j;
      }
    }
...
#pragma omp parallel for private(i)
      ...
      for (i = 0; i < nnz_offd; i++) {
        offd_j[i] =
            hypre_BigBinarySearch(col_map_offd, big_offd_j[i], num_cols_offd);
      }
...
HYPRE_Int hypre_IJMatrixSetValuesOMPParCSR(hypre_IJMatrix *matrix,
                                           HYPRE_Int nrows, HYPRE_Int *ncols,
                                           const HYPRE_BigInt *rows,
                                           const HYPRE_Int *row_indexes,
                                           const HYPRE_BigInt *cols,
...
else
{
  ...
  {
    printf("start assembling of matrix \n");
    ...
    if (need_aux) {
      ...
      if (num_procs > 1) {
        ...
        if (!big_offd_j) {
          big_offd_j =
              hypre_CTAlloc(HYPRE_BigInt, offd_i[hypre_CSRMatrixNumRows(offd)],
                            hypre_CSRMatrixMemoryLocation(offd));
          hypre_CSRMatrixBigJ(offd) = big_offd_j;
        }
      }
      ...
      for (ii = ns; ii < ne; ii++) {
        ...
        if (row >= row_partitioning[pstart] &&
          ...
          if (need_aux) {
            ...
            if (tmp_indx) {
              aux_j[row_local] =
                  hypre_TReAlloc(aux_j[row_local], HYPRE_BigInt,
                                 size + tmp_indx, HYPRE_MEMORY_HOST);
              aux_data[row_local] =
                  hypre_TReAlloc(aux_data[row_local], HYPRE_Complex,
                                 size + tmp_indx, HYPRE_MEMORY_HOST);
...
HYPRE_Int hypre_IJMatrixAddToValuesOMPParCSR(hypre_IJMatrix *matrix,
                                             HYPRE_Int nrows, HYPRE_Int *ncols,
                                             const HYPRE_BigInt *rows,
                                             const HYPRE_Int *row_indexes,
                                             const HYPRE_BigInt *cols,
...
#pragma omp atomic
  ...
  if (!aux_matrix) {
    HYPRE_Int size =
        (HYPRE_Int)(row_partitioning[pstart + 1] - row_partitioning[pstart]);
    hypre_AuxParCSRMatrixCreate(&aux_matrix, size, size, NULL);
    hypre_AuxParCSRMatrixNeedAux(aux_matrix) = 0;
    hypre_IJMatrixTranslator(matrix) = aux_matrix;
  }
