...
HYPRE_Int HYPRE_IJMatrixCreate(MPI_Comm comm, HYPRE_BigInt ilower,
                               HYPRE_BigInt iupper, HYPRE_BigInt jlower,
                               HYPRE_BigInt jupper, HYPRE_IJMatrix *matrix) {
  ...
  row_partitioning =
      hypre_CTAlloc(HYPRE_BigInt, num_procs + 1, HYPRE_MEMORY_HOST);
  ...
  hypre_MPI_Allgather(info, 4, HYPRE_MPI_BIG_INT, recv_buf, 4,
  ...
  hypre_IJMatrixGlobalNumRows(ijmatrix) =
      row_partitioning[num_procs] - row_partitioning[0];
  hypre_IJMatrixGlobalNumCols(ijmatrix) =
      col_partitioning[num_procs] - col_partitioning[0];
  ...
  if (ijmatrix) {
    ...
    if hypre_IJMatrixAssumedPart (ijmatrix) {
      hypre_AssumedPartitionDestroy(
          (hypre_IJAssumedPart *)hypre_IJMatrixAssumedPart(ijmatrix));
    }
...
HYPRE_Int HYPRE_IJMatrixInitialize_v2(HYPRE_IJMatrix matrix,
...
HYPRE_Int hypre_PrefixSumInt(HYPRE_Int nvals, HYPRE_Int *vals,
                             HYPRE_Int *sums) {
  HYPRE_Int j, nthreads, bsize;

  nthreads = hypre_NumThreads();
  bsize =
      (nvals + nthreads - 1) / nthreads; /* This distributes the remainder */
...
#pragma omp parallel for private(j) HYPRE_SMP_SCHEDULE
    ...
    /* Compute final partial sums (in serial) for the first entry of every
     * interval */
...
HYPRE_Int HYPRE_IJMatrixSetValues(HYPRE_IJMatrix matrix, HYPRE_Int nrows,
...
/*--------------------------------------------------------------------------
 *--------------------------------------------------------------------------*/
HYPRE_Int HYPRE_IJMatrixSetValues2(HYPRE_IJMatrix matrix, HYPRE_Int nrows,
                                   HYPRE_Int *ncols, const HYPRE_BigInt *rows,
  ...
  */
  ...
  HYPRE_ExecutionPolicy exec =
      hypre_GetExecPolicy1(hypre_IJMatrixMemoryLocation(matrix));

  if (exec == HYPRE_EXEC_DEVICE) {
    hypre_IJMatrixSetAddValuesParCSRDevice(ijmatrix, nrows, ncols, rows,
                                           row_indexes, cols, values, "set");
  } else
#endif
  {
...
HYPRE_Int HYPRE_IJMatrixAddToValues(HYPRE_IJMatrix matrix, HYPRE_Int nrows,
                                    HYPRE_Int *ncols, const HYPRE_BigInt *rows,
                                    const HYPRE_BigInt *cols,
                                    const HYPRE_Complex *values) {
  hypre_IJMatrix *ijmatrix = (hypre_IJMatrix *)matrix;
...
HYPRE_Int HYPRE_IJMatrixAddToValues2(HYPRE_IJMatrix matrix, HYPRE_Int nrows,
                                     HYPRE_Int *ncols, const HYPRE_BigInt *rows,
                                     const HYPRE_Int *row_indexes,
                                     const HYPRE_BigInt *cols,
                                     const HYPRE_Complex *values) {
  ...
  */
...
#endif
  {
    ...
    if (hypre_IJMatrixOMPFlag(ijmatrix)) {
      hypre_IJMatrixAddToValuesOMPParCSR(ijmatrix, nrows, ncols_tmp, rows,
                                         row_indexes_tmp, cols, values);
    } else {
      hypre_IJMatrixAddToValuesParCSR(ijmatrix, nrows, ncols_tmp, rows,
                                      row_indexes_tmp, cols, values);
    }
...
HYPRE_Int HYPRE_IJMatrixGetRowCounts(HYPRE_IJMatrix matrix, HYPRE_Int nrows,
...
HYPRE_Int HYPRE_IJMatrixGetValues(HYPRE_IJMatrix matrix, HYPRE_Int nrows,
  ...
  if (hypre_IJMatrixObjectType(ijmatrix) == HYPRE_PARCSR) {
    hypre_IJMatrixGetValuesParCSR(ijmatrix, nrows, ncols, rows, cols, values);
  } else {
    hypre_error_in_arg(1);
  }
...
HYPRE_Int HYPRE_IJMatrixRead(const char *filename, MPI_Comm comm,
...
HYPRE_Int HYPRE_IJMatrixPrint(HYPRE_IJMatrix matrix, const char *filename) {
  ...
  for (i = ilower; i <= iupper; i++) {
...
#endif
      HYPRE_ParCSRMatrixGetRow((HYPRE_ParCSRMatrix)object, ii, &ncols, &cols,
                               &values);
      for (j = 0; j < ncols; j++) {
#ifdef HYPRE_NO_GLOBAL_PARTITION
    ...
    if (hypre_IJMatrixObjectType(matrix) == HYPRE_PARCSR) {
      ...
      HYPRE_ParCSRMatrixRestoreRow((HYPRE_ParCSRMatrix)object, ii, &ncols,
                                   &cols, &values);
    }
  }
