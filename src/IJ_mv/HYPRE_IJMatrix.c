...
hypre_PrefixSumInt(HYPRE_Int nvals,
                   ...
                   HYPRE_Int *sums)
{
   HYPRE_Int j, nthreads, bsize, tmp;
                   ...
                   HYPRE_IJMatrix *matrix_ptr)
{
   ...
   ncols = 1;
   while ((ret = hypre_fscanf(file, "%b %b%*[ \t]%le", &II, &JJ, &value)) != EOF)
   {
      if (ret != 3)
      {
      ...
      if (II < ilower || II > iupper)
      {
         HYPRE_IJMatrixAddToValues(matrix, 1, &ncols, &II, &JJ, &value);
      }
      else
      {
         HYPRE_IJMatrixSetValues(matrix, 1, &ncols, &II, &JJ, &value);
      }
   }
